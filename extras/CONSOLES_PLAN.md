...
| **2. NES** (**SMB and SMB3 (MMC3, from PSRAM) play on the Fruit Jam in all four rotations**, 2026-09-26, DEVNOTES #136; still to do: battery saves, STRETCH aspect correction) | **Spike first:** a host harness running nofrendo and fixNES on blargg's test ROMs, with frame timing (`extras/tools/nes_test`), then nofrendo on the Fruit Jam with Super Mario Bros. against the 16.7 ms budget. Then port the chosen core onto the HAL (the first GPL core): PicoDVI video at 252 MHz, our audio and input, console geometry. The core brings its mappers with it (nofrendo: 59), so there's no mapper-by-mapper build-up. | blargg's NES CPU tests pass on the host harness. Super Mario Bros. (NROM) and an MMC3 game run at 60 fps on the Fruit Jam with sound, **in all four rotations**. The `nm` check passes on every arcade binary. |
