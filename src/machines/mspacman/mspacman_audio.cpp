...
// mspacman_ports.cpp already masks each write to 0x0F on the way in, so no
...
// that same accumulator at MSPACMAN_AUDIO_SAMPLE_RATE instead of the real
...
// waveform_r()): signed_sample = (mspacman_wave_prom[select*32 + position]
...
static void ARCADE_FAST_FUNC(mspacman_audio_fill)(int32_t *out, int count) {
    ...
    for (int i = 0; i < MSPACMAN_SOUND_REG_SIZE; i++) regs[i] = g_system->sound_regs[i];
    ...
    // step = frequency * (WSG_CLOCK_HZ / MSPACMAN_AUDIO_SAMPLE_RATE), scaled
    // up by 2^FRAC_BITS for fixed-point precision. 64-bit intermediate
    // avoids any overflow risk (frequency is at most 20 bits).
    step[0] = (uint32_t)(((uint64_t)freq0 * WSG_CLOCK_HZ << FRAC_BITS) / MSPACMAN_AUDIO_SAMPLE_RATE);
    step[1] = (uint32_t)(((uint64_t)freq1 * WSG_CLOCK_HZ << FRAC_BITS) / MSPACMAN_AUDIO_SAMPLE_RATE);
    step[2] = (uint32_t)(((uint64_t)freq2 * WSG_CLOCK_HZ << FRAC_BITS) / MSPACMAN_AUDIO_SAMPLE_RATE);
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int v = 0; v < 3; v++) {
            ...
            int sample = (int)(mspacman_wave_prom[(waveform[v] << 5) | pos] & 0x0F) - 8;
...
void mspacman_audio_init(mspacman_system *system) {
    g_system = system;
    hal_audio_set_fill_callback(mspacman_audio_fill);
}
